...
Un panou de administrare care rulează local pe calculatorul tău, ca să gestionezi
catalogul vizual (fără să editezi JSON de mână).

### Pornire
```
npm run admin
```
Apoi deschide în browser: **http://localhost:4321**

### Ce poți face
- **Jocuri**: caută/filtrează (categorie, featured, orientare), editează titlu/descriere/
  categorie/orientare/tag-uri, marchează Featured ⭐, șterge jocuri, reordonează
  (▲▼ sau scrie poziția) — controlezi manual ce apare primul.
- **Categorii**: redenumește sau contopește categorii (ex. unești ".io" cu "IO").
  Se aplică automat tuturor jocurilor din acea categorie.
- **Import premium**: trage fișierele JSON din RSS Builder direct în panel.
  Se contopesc peste baza existentă cu scor de calitate (cele care apar în mai
  multe liste urcă sus). Numele fișierului dă greutatea (editor > exclusive > best > hot > popular).

### După modificări
1. Apasă **💾 Salvează games.json** în panel (face backup automat în `admin/backups/`).
2. Rulează `node build.js` ca să regenerezi site-ul.
3. Verifică local cu `npx serve dist`, apoi `git push`.

> Folderul `admin/` rulează DOAR local — nu ajunge niciodată pe site (build-ul
> copiază doar `static/` în `dist/`). Backup-urile din `admin/backups/` sunt în .gitignore.
